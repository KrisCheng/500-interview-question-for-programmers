...
* **描述Java内存模型（阿里）**
	* JVM内存区域划分
		...
		* 虚拟机栈（Virtual Machine Stack） --> 保存一个个栈帧（Stack Frame），对应着一次次的Java方法调用
...
* **描述Java的GC过程（DaoCloud）**
	* 对象存活判断
		* 引用计数（Python 的 GC），无法解决对象相互循环引用的问题，Java中没有使用
...
* **拼多多学霸批服务端 0728**
	* 1. 严格升序数组替换
		* 需要考虑替换哪一个数，逆序对需要提取出来，而不是仅仅考虑逆序对中小的那个，如：```1, 3, 8, 7, 10```， 替换8或者替换7都有可能(可以先考虑替换n+1，不满足再替换n)；
