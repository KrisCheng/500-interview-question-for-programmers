...
* **手写主流排序算法 & 各种算法的复杂度/稳定性分析（腾讯/字节跳动/阿里/蜻蜓FM/Unity）**
  ...
  * [堆排 Wiki](https://zh.wikipedia.org/wiki/%E5%A0%86%E6%8E%92%E5%BA%8F)（ 堆排的代码实现）
  * [堆排序(Heapsort)](https://www.youtube.com/watch?v=j-DqQcNPGbE) （特别好的讲解）
  * [归并排序 Wiki](https://zh.wikipedia.org/wiki/%E5%BD%92%E5%B9%B6%E6%8E%92%E5%BA%8F)（ 递归版本和循环版本的代码实现）
...
* **（项目相关）个人项目的查询引擎设置 AbstractProvider 这种抽象类用到了什么模式（星环）**
  ...
  * [模板模式]()
* **设计一个延迟在10ms以内，QPS 在10000的服务，需要考虑哪些点，或者怎么设计（得物）**

  * 这都是什么题啊。。。

* **如果当前系统访问量（如QPS，订单量）扩展10倍，有哪些方面需要考量（Shopee）**
* **设计一个服务，用于实时统计一个直播间的在线人数（Shopee）**
* **有哪些常用的负载均衡算法（策略）（Shopee）**
